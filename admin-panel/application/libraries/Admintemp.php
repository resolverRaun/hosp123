 ...
 * @category	Libraries
 * @link		http://www.williamsconcepts.com/ci/libraries/admintemp/index.html
...
class CI_Admintemp {
   ...
   var $parse_admintemp = FALSE;
   
   /**
	 * Constructor
	 *
	 * Loads admintemp configuration, admintemp regions, and validates existence of 
	 * default admintemp
	 *
	 * @access	public
	 */
   
   function CI_Admintemp()
   {
      // Copy an instance of CI so we can use the entire framework.
      $this->CI =& get_instance();
      
      // Load the admintemp config file and setup our master admintemp and regions
      include(APPPATH.'config/admintemp'.EXT);
      if (isset($admintemp))
      {
         $this->config = $admintemp;
         $this->set_admintemp($admintemp['active_admintemp']);
      }
   }
   ...
   /**
    * Use given admintemp settings
    *
    * @access  public
    * @param   string   array key to access admintemp settings
    * @return  void
    */
   
   function set_admintemp($group)
   {
      if (isset($this->config[$group]))
      {
         $this->admintemp = $this->config[$group];
      }
      else
      {
         show_error('The "'. $group .'" admintemp group does not exist. Provide a valid group name or add the group first.');
      }
      $this->initialize($this->admintemp);
   }
   
      // --------------------------------------------------------------------
   
   /**
    * Set master admintemp
    *
    * @access  public
    * @param   string   filename of new master admintemp file
    * @return  void
    */
   
   function set_master_admintemp($filename)
   {
   ...
   /**
    * Dynamically add a admintemp and optionally switch to it
    *
    * @access  public
    * @param   string   array key to access admintemp settings
   ...
   function add_admintemp($group, $admintemp, $activate = FALSE)
   {
      if ( ! isset($this->config[$group]))
      {
         $this->config[$group] = $admintemp;
         if ($activate === TRUE)
         {
            $this->initialize($admintemp);
         }
      }
      else
      {
         show_error('The "'. $group .'" admintemp group already exists. Use a different group name.');
      }
   }
   ...
   {
      // Set master admintemp
      if (isset($props['admintemp']) 
         && (file_exists(APPPATH .'views/'. $props['admintemp']) or file_exists(APPPATH .'views/'. $props['admintemp'] . EXT)))
      {
         $this->master = $props['admintemp'];
      }
      else 
      {
         // Master admintemp must exist. Throw error.
         show_error('Either you have not provided a master admintemp or the one provided does not exist in <strong>'. APPPATH .'views</strong>. Remember to include the extension if other than ".php"');
      }
      ...
      // Set master admintemp parser instructions
      $this->parse_admintemp = isset($props['parse_admintemp']) ? $props['parse_admintemp'] : FALSE;
   }
   ...
   function set_regions($regions)
   {
      if (count($regions))
      {
         ...
         {
            // Regions must be arrays, but we take the burden off the admintemp 
   ...
   /**
    * Dynamically add region to the currently set admintemp
    *
    * @access  public
   ...
   /**
    * Dynamically include javascript in the admintemp
    * 
   ...
   /**
    * Dynamically include CSS in the admintemp
    * 
   ...
   /**
	 * Render the master admintemp or a single region
	 *
	 * @access	public
	 * @param	string	optionally opt to render a specific region
	 * @param	boolean	FALSE to output the rendered admintemp, TRUE to return as a string. Always TRUE when $region is supplied
	 * @return	void or string (result of admintemp build)
	 */
   ...
   function render($region = NULL, $buffer = FALSE, $parse = FALSE)
   {
      ...
      else
      {
         ...
         if ($this->parse_admintemp === TRUE or $parse === TRUE)
         {
            // Use provided parser class and method to render the admintemp
         ...
         {
            // Use CI's loader class to render the admintemp with our output array
   ...
   /**
    * Load the master admintemp or a single region
    *
    * DEPRECATED!
    * 
    * Use render() to compile and display your admintemp and regions
    */
   ...
   function _build_content($region, $wrapper = NULL, $attributes = NULL)
   {
      ...
      if (isset($region['wrapper'])) 
      {
         ...
         if (isset($region['attributes']) && is_array($region['attributes']))
         {
            ...
            {
               // We don't validate HTML attributes. Imagine someone using a custom XML admintemp..
               $output .= " $name=\"$value\"";
            }
         }
...
/* End of file Admintemp.php */
/* Location: ./system/application/libraries/Admintemp.php */